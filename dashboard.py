        ...
        if "Kimyasal" in mol_type:
            with st.expander("🧪 Genişletilmiş Atom Renk Rehberi", expanded=True):
                ...
                * 🟢 **Yeşil:** Klor/Flor | 🟤 **Koyu Kırmızı:** Brom | 🟣 **Mor:** İyot | 🪨 **Gri:** Demir
                """)
            with st.expander("⚠️ NFPA 704 Kimyasal Tehlike Elması", expanded=False):
                st.markdown("🟦 Sağlık (0-4) | 🔴 Yanıcılık (0-4) | 🟡 Reaktivite (0-4) | ⬜ Özel (OX, ₩, COR, BIO, RAD, POI)")
        else:
...
with tab9:
    ...
    st.markdown("İki farklı biyolojik varlığı (Örn: Bir virüs proteini ile insan reseptörü veya bir ilaç) sanal arenada bir araya getirin. Sistem, aynı uzayda yapıları çarpıştırır ve Yapay Zeka bu birleşmenin biyolojik sonucunu tahmin eder.")
    ...
    if st.button("💥 Çarpıştır ve Etkileşimi Analiz Et", type="primary"):
        ...
        # 1. Aşama: 3D Görselleştirme (Aynı uzayda bindirme)
        st.markdown("### 🔬 3D Moleküler Çarpışma Arenası")
        try:
            view = py3Dmol.view(width=800, height=500)
            
            # Varlık A'yı Yükle (Kırmızı)
            if pdb_a:
        ...
        # 2. Aşama: DSPy (LLM) ile Biyolojik Analiz
